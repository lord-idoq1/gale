...
const { pindl, likeedl, snapchatdl, IGVideo, IGReels, IGStory, IGPhoto, fbdl } = require('../lib/rugel');
...
fetch(encodeURI(`https://api-idoganz1.herokuapp.com/downloader/igStory?username=${username}`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({             
...
router.get('/random/tiktok', async(req, res, next) => {
  const apikey = req.query.apikey;
        if (apikey === undefined) return res.status(404).send({
            status: 404,
            message: `Input Parameter apikey`
      ...
      limitAdd(apikey);   
fetch(encodeURI(`https://api-idoganz1.herokuapp.com/search/randomtiktok`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({             
...
router.get('/instagram/video', async(req, res, next) => {
  const url = req.query.url
  const apikey = req.query.apikey;
        if (apikey === undefined) return res.status(404).send({
            status: 404,
...
router.get('/instagram/real', async(req, res, next) => {
  const url = req.query.url
  const apikey = req.query.apikey;
        if (apikey === undefined) return res.status(404).send({
            status: 404,
...
router.get('/instagram/photo', async(req, res, next) => {
  const url = req.query.url
  const apikey = req.query.apikey;
        if (apikey === undefined) return res.status(404).send({
            status: 404,
