...
const { pinterestdl } = require('../lib/pinterestdl');
const { scdl } = require('../lib/scdl');
const { stickerDl, stickerSearch } = require('../lib/stickerpack');
const { konachan } = require('../lib/konachan');
const { tiktok } = require('../lib/tiktokk');
const { randomTiktok } = require('../lib/tiktok-search');
const { photoOxy } = require('./oxy');
const { randomTiktok } = require('../tiktok-search');
...
router.get('/pindl', async(req, res) => {
  ...
  if (!url) return res.json(loghandler.invalidlink);
	var hasil = await pinterestdl(url)
	try {
		res.json(hasil)
	} catch(err) {
		console.log(err)
