...
const q = req.query.q
  const apikey = req.query.apikey;
      ...
      limitAdd(apikey);
   if(!q) return res.json({creator: 'DappaUhuy', status: true, message: 'masukan parameter q})
fetch(encodeURI(`https://https://h4ck3rs404-api.herokuapp.com/api/phub-search?q=${q}&apikey=404Api`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({             
...
const q = req.query.q
  const apikey = req.query.apikey;
      ...
      limitAdd(apikey);
  if(!q) return res.json({creator: 'DappaUhuy', status: true, message: 'masukan parameter q})
fetch(encodeURI(`https://h4ck3rs404-api.herokuapp.com/api/xvids-search?q=${q}&apikey=404Api`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({             
...
const q = req.query.q
  const apikey = req.query.apikey;
      ...
      limitAdd(apikey);
    if(!q) return res.json({creator: 'DappaUhuy', status: true, message: 'masukan parameter q})
fetch(encodeURI(`https://h4ck3rs404-api.herokuapp.com/api/xnxx-search?q=${q}&apikey=404Api`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({             
...
router.get('/imgedit/pornhub', async(req, res, next) => {
  const username = req.query.username
    const ppurl = req.query.ppurl;
...
if(!ppurl) return res.json({creator: 'DappaUhuy', status: true, message: 'masukan parameter ppurl})
if(!msg) return res.json({creator: 'DappaUhuy', status: true, message: 'masukan parameter msg})
  let hasil = `https://h4ck3rs404-api.herokuapp.com/api/phubcomen?username=$username}&ppurl=${url}&msg=&{msg}&apikey=404Api`
  data = await fetch(hasil).then(v => v.buffer())
         await fs.writeFileSync(__path +'/tmp/tod.png', data)
        res.sendFile(__path+'/tmp/tod.png')
});
